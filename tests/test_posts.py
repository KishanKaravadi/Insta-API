...
def test_get_all_my_posts(authorized_client, test_posts):
    res = authorized_client.get('/posts/me')
...
def test_unauthorized_user_get_my_posts(client, test_posts):
    res = client.get('/posts/me')
    assert res.status_code == 401


def test_get_one_post_not_exist(client, test_posts):
    res = client.get('/posts/234234234')
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post(
        '/posts/', json={'title': 'hi', 'content': 'bye'})

    created_post = schemas.Post(**res.json())

    assert res.status_code == 201
    assert created_post.title == 'hi'
    assert created_post.content == 'bye'
